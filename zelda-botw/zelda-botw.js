...
function addToMapNum(data, icon, num){
	...
	var count = 0;
	for (var i = 0; i<points.length && mapPinCount<MAX_MAP_PINS && count<num; i++){
		var pt = points[i]
		var hash = pt.H;
		var offset=SavegameEditor._searchHash(hash);
		if(offset && !tempFile.readU32(offset + 4)){
...
function addKoroksToMap(num){
	var n = addToMapNum(BOTW_Data.KOROKS, SavegameEditor.Constants.ICON_TYPES.LEAF, num);
